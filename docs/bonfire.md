...
Session actions contain only a nonce and `SELECT_FEATURE + featureId` or `LEAVE`; the client never decides when Rest succeeds. Each action and server tick rechecks placement generation, dimension, reach, life/spectator state, role, active boss and hostile session. Feature execution also rechecks registration, current configuration and availability. SOLO/HOST may use bonfires; COOPERATOR/INVADER may not activate, rest or level up in a foreign world. Animation and fade completion are not gameplay authorities.

Features now persist as up to 64 namespaced IDs (128 characters each). Old short names migrate to `maplesadventure:`; valid unknown addon IDs remain saved while their mod is absent. The server constructs menu rows from configured, registered and available handlers, in order then lexical-ID order. Level Up is registered internally; Flask/Spell Memory/Reinforce/Warp are reserved but hidden until implemented. New rows reuse the existing menu style and fade effects. See [Bonfire API](integration/bonfire-api.md).
...
On normal death, NeoForge `PlayerRespawnPositionEvent` validates the stored dimension and exact generation, then searches safe nearby positions (radius at most three blocks, vertical offset at most two). It checks solid footing, collision, fluids and the full standing bounding box against the world border, and creates no persistent chunk ticket. Removed/replaced placements clear lastRested; temporarily obstructed safe positions preserve it. Both cases fall back to Vanilla for this respawn. Lost Soul and foreign-player phantom death are not modified. A successful Rest calls the built-in encounters participant through BonfirePhaseResetService once; it delegates to `EncounterResetService.resetForPlayer(..., BONFIRE)`. Other phases, shared mobs and debug prototype mobs are untouched. Opening, upgrading and leaving never reset encounters.
